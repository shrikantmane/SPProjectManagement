...
export default class Activities extends React.Component<IActivityProps, IActivityState> {
    ...
    public componentDidMount() {
        this._getActivityDetails(this.props.projectId,this.props.taskId);
    }
    public componentWillReceiveProps(nextProps) {
        this._getActivityDetails(nextProps.projectId,nextProps.projectId);
    }

    private async _getActivityDetails(projectId,taskId): Promise<void> {
        let items;
        if(projectId != null){
            items = await pnp.sp.web.lists.getByTitle('Activity Log').items.filter(`ProjectName/Id eq '${projectId}'`).select("ProjectName/Id","Activity_x0020_Date","Activity_x0020_By/Title","Task_x0020_Name/Title","Activity_x0020_For","Old_x0020_Value", "New_x0020_Value")
            .expand("Activity_x0020_By","Task_x0020_Name","ProjectName").orderBy("Modified", false).get();
        }
        else if(taskId != null){
            items = await pnp.sp.web.lists.getByTitle('Activity Log').items.filter(`Task_x0020_Name/Id eq '${taskId}'`).select("Activity_x0020_Date","Activity_x0020_By/Title","Task_x0020_Name/Title","Activity_x0020_For","Old_x0020_Value", "New_x0020_Value")
