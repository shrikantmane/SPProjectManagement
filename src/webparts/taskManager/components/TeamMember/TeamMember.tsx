...
// dilogue
  ...
  private _onRefreshItems(): void {
    this._getListItems(this.state.projectId);
  }
  ...
  componentWillReceiveProps(nextProps){
    ...
    this._getListItems(nextProps.projectId);
  }

   private _getListItems(projectId): void {
    console.log("this.props.projectId - TM", projectId);
    if(projectId === null)
      return;
    let list = sp.web.lists.getByTitle("Project Team Member");
       list.items
      .select("ID","ProjectID", "StartDate", "EndDate", "Status","TeamMember/Title", "TeamMember/ID", "Project/Title", "Project/ID").expand("TeamMember","Project").filter("Project/ID eq " + projectId + " and Status eq 'Active'")
      .get()
      .then((response) => {
        this.setState({
          items: response
